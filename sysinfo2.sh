...
writeFoot(){
echo "<p style=\"text-align:center\">$PBY</p>"
echo "<p>Finished at $(date -R).</p>"
echo "</body></html>"
}
...
echo "<h2>Host: <a href=\"//$myhost\">$myhost</a></h2>"
...
numcores=$(cat /proc/cpuinfo | grep 'model name' | wc -l)
...
rfs="$(LANG=C df -hTl 2>/dev/null | grep -vE "^Filesystem|shm" |
 awk ' \
  {w=sprintf("%d",$6); \
   if (w > 90) {
...
rusedswap="$(LANG=C free -mt | grep Swap: | awk '{ print $3 }')"
rfreeswapout="$(LANG=C free -mt | grep Swap: | awk '{ print $4  }')"
rtotalswap="$(LANG=C free -mt | grep Swap: | awk '{ print $2 }')"
...
#user_processes_body=$(LANG=C ps hax -o user | sort | uniq -c | grep "$(awk -F':' '{ if($3 >= 1000 && $3 < 10000) print $1 }' /etc/passwd | sed -e 's/\n/|/g')" | sed 's/^ *//;s/ *$//' | awk -F" " '{print "<tr><td>" $1 "</td><td>" $2 "</td></tr>"}')
...
# Check if rebooting is recommended
REBOOTNEEDEDCMD='ls /var/run/reboot-required >/dev/null 2>/dev/null && echo "<span style=\"color:red\">Yes</span>" || echo "No"'
REBOOT_NEEDED="$($REBOOTNEEDEDCMD)"
